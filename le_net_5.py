...
class LeNet5(nn.Module):
    ...
    def forward(self, x):
        x = self.scaled_tanh(self.conv1(x))     # Apply C1 + activation
        x = self.pool1(x)                       # Apply S2
        x = self.scaled_tanh(self.conv2(x))     # C3
        x = self.pool2(x)                       # S4
        x = self.scaled_tanh(self.conv3(x))     # C5
        x = x.view(-1, 120)                     # Flatten
        x = self.scaled_tanh(self.fc1(x))       # F6
